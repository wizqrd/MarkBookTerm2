...
def add_student():
    ...
    while not Verify_Name(firstName):
...
def Verify_Name(name):
   ...
   def addStudent():
       ...
       studentID = input("Enter student ID please: ")
       if not isValidStudentID(studentID):
           print("Invalid student ID. Please enter a 5 digit numerical string.")
           return
   ...
   def displayMarkbook():
       print("{:<10} {:<15} {:<15} {:<20} {:<5}".format('Student ID',
       ...
       for studentID, studentData in markbook.items():
           ...
           print("{:<10} {:<15} {:<15} {:<20} {:<5}".format(
               studentID, firstName, lastName, marks, grade))
   ...
   def saveMarkbook():
       ...
       try:
           with open(f"{filename}.txt", 'w') as file:
               for studentID, studentData in markbook.items():
                   file.write(
                       f"{studentID},{studentData['firstName']},{studentData['lastName']},{','.join(str(mark) for mark in studentData['marks'])},{studentData['grade']}\n"
                   )
   ...
   def loadMarkbook():
       ...
       try:
           with open(f"{filename}.txt", 'r') as file:
               markbook.clear()
               for line in file:
                   studentID, firstName, lastName, marksStr, grade = line.strip(
                   ).split(',')
